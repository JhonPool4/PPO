...
class InvertedPendulum(Env):
    def __init__(self):
        ...
        max_obs_values= np.array([self.max_angle, self.max_speed], dtype=np.float32)
        self.observation_space = Box(low=-max_obs_values, high=max_obs_values)      

    def reset(self):
        # initial configuration: with normalization
        init_ang = (np.random.uniform(-self.max_angle, self.max_angle))%(2*np.pi)
        init_vel = np.random.uniform(-5*np.pi/180, 5*np.pi/180)
        self.states = np.array([init_ang, init_vel], dtype=np.float32)
        # simulation time
        self.sim_time = 500
    ...
    def step(self, act):        
        ...
        # velocity
        new_dth = np.clip(dth + self.dt*new_ddth, -self.max_speed, self.max_speed)
        # position: with normalization
        new_th = (th + self.dt*(new_dth) + 0.5*np.power(self.dt,2)*new_ddth)%(2*np.pi)

        # update states
        self.states= np.array([new_th, new_dth], dtype=np.float32)
        
        # reward system:
        #alive_bonus = np.power(0.5*np.pi/180,2)
        cost_e = -1*np.power(self.th_des - th, 2) # reduce positin error
        cost_de = -0.1*np.power(self.dth_des - dth,2) # reduce velocity error
        #cost_dde = -0.001*np.power(self.ddth_des - ddth, 2) # reduce acceleration error
        cost_tau = -0.001*np.power(self.tau,2) # reduce energy consumption
        # reward
        reward = cost_e + cost_de + cost_tau #+ alive_bonus
    ...
    def render(self):
        if self.flag_render:
            ...
            text_e = self.font.render("Error: "+str(np.round((self.th_des-self.states[0])*180/np.pi,2)), True, (0,0,255))
            ...
            text_de = self.font.render("dError: "+str(np.round((-self.states[1])*180/np.pi,2)), True, (0,0,255))
